    ...
    <div className="p-8 max-w-4xl mx-auto space-y-6">
      <div className="pb-6 border-b border-slate-200 dark:border-slate-800">
        <h1 className="text-2xl font-extrabold text-slate-800 dark:text-slate-100 tracking-tight flex items-center gap-2">
          <KeyRound className="w-7 h-7 text-indigo-600" /> API Diagnostic & Model Status
        </h1>
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
          Monitor endpoint health, server status, and run test queries against Gemini AI models.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 space-y-4 shadow-sm">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
              <Activity className="w-4 h-4 text-emerald-500" /> Server Health
            </h3>
            <button
              onClick={checkHealth}
          ...
          <div className="space-y-2 text-xs">
            <div className="flex justify-between py-1 border-b border-slate-100 dark:border-slate-800">
              <span className="text-slate-400">Server Route:</span>
              <span className="font-semibold text-slate-700 dark:text-slate-300">Express / Node.js</span>
            </div>
            <div className="flex justify-between py-1 border-b border-slate-100 dark:border-slate-800">
              <span className="text-slate-400">Host Binding:</span>
              <span className="font-semibold text-slate-700 dark:text-slate-300">0.0.0.0:3000</span>
            </div>
            <div className="flex justify-between py-1 border-b border-slate-100 dark:border-slate-800">
              <span className="text-slate-400">Gemini Key Injected:</span>
              <span className="font-bold text-emerald-500">
                {healthData?.geminiKeyConfigured ? 'Yes (Active)' : 'No'}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 space-y-4 shadow-sm">
          <h3 className="text-sm font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
            <Zap className="w-4 h-4 text-amber-500" /> Run Live Gemini Test
          </h3>

          <p className="text-xs text-slate-400">
            Sends a test translation string to `gemini-3.6-flash` via the Express server.
          </p>
          ...
          >
            {testing ? <RefreshCw className="w-4 h-4 animate-spin" /> : 'Run Test Request'}
          </button>
          ...
          {testResult && (
            <div className="p-3 rounded-xl bg-indigo-50/60 dark:bg-indigo-950/40 border border-indigo-500/20 text-xs font-semibold text-slate-800 dark:text-slate-200">
              <span className="text-indigo-500 text-[10px] block font-bold mb-1">RESPONSE:</span>
              {testResult}
            </div>
          )}
        </div>
